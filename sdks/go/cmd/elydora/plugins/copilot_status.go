...
func readCopilotRuntimeConfig(path string) (map[string]any, bool, error) {
...
func requireCopilotRuntimeString(
...
func validateCopilotRuntimeConfig(
	config map[string]any,
	...
	if _, err := requireCopilotRuntimeString(config, "org_id", configPath); err != nil {
		return err
	}
	if _, err := requireCopilotRuntimeString(config, "kid", configPath); err != nil {
		return err
	}
	agentID, err := requireCopilotRuntimeString(config, "agent_id", configPath)
	if err != nil {
		return err
	}
	if !sameCopilotAgentID(agentID, expectedAgentID) ||
		config["agent_name"] != copilotAgentKey {
		return fmt.Errorf(
			"Elydora runtime identity does not match Copilot hooks: %s",
			configPath,
		)
	}
	if _, exists := config["token"]; exists {
		if _, err := requireCopilotRuntimeString(config, "token", configPath); err != nil {
			return err
		}
	}
	baseURL, err := requireCopilotRuntimeString(config, "base_url", configPath)
	if err != nil {
		return err
	}
...
func validateCopilotRuntimeIdentity(agentDirectory, agentID string) error {
	...
	config, configExists, err := readCopilotRuntimeConfig(configPath)
	if err != nil {
		return err
	}
	...
	for _, item := range []struct {
		...
		{filepath.Join(agentDirectory, copilotGuardScript), "Elydora guard runtime", maxManagedSourceBytes},
		{filepath.Join(agentDirectory, copilotAuditScript), "Elydora audit runtime", maxManagedSourceBytes},
	...
	if !ok || config["agent_name"] != copilotAgentKey ||
		!sameCopilotAgentID(configuredID, agentID) {
		return fmt.Errorf(
			"Elydora runtime config identity does not match Copilot agent %s: %s",
			agentID,
			configPath,
		)
	}
...
func validCopilotContractPaths(contract copilotRuntimeContract) (bool, error) {
	runtimeRoot, err := AgentRuntimeRoot()
	if err != nil {
		return false, err
	}
	agentDirectory := filepath.Dir(contract.guardPath)
	return sameCopilotPath(filepath.Dir(agentDirectory), runtimeRoot) &&
		sameCopilotPath(
			contract.guardPath,
			filepath.Join(agentDirectory, copilotGuardScript),
		) &&
		sameCopilotPath(
			contract.auditPath,
			filepath.Join(agentDirectory, copilotAuditScript),
		), nil
}

func copilotRuntimeContractExists(contract copilotRuntimeContract) (bool, error) {
	validPaths, err := validCopilotContractPaths(contract)
	...
	if err := validateCopilotRuntimeConfig(config, contract.agentID, configPath); err != nil {
		return false, err
	}
...
func copilotRuntimeFilesExist(contracts []copilotRuntimeContract) (bool, error) {
	for _, contract := range contracts {
		exists, err := copilotRuntimeContractExists(contract)
		if err != nil {
			return false, err
		}
		if exists {
