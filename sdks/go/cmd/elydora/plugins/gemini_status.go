...
func readGeminiRuntimeConfig(path string) (map[string]any, bool, error) {
...
func requireGeminiRuntimeString(
...
func validateGeminiRuntimeConfig(
	config map[string]any,
	...
	if _, err := requireGeminiRuntimeString(config, "org_id", configPath); err != nil {
		return err
	}
	if _, err := requireGeminiRuntimeString(config, "kid", configPath); err != nil {
		return err
	}
	agentID, err := requireGeminiRuntimeString(config, "agent_id", configPath)
	if err != nil {
		return err
	}
	if !sameGeminiAgentID(agentID, expectedAgentID) ||
		config["agent_name"] != geminiAgentKey {
		return fmt.Errorf(
			"elydora runtime identity does not match Gemini CLI hooks: %s",
			configPath,
		)
	}
	if _, exists := config["token"]; exists {
		if _, err := requireGeminiRuntimeString(config, "token", configPath); err != nil {
			return err
		}
	}
	baseURL, err := requireGeminiRuntimeString(config, "base_url", configPath)
	if err != nil {
		return err
	}
...
func validateGeminiRuntimeIdentity(agentDirectory, agentID string) error {
	...
	config, configExists, err := readGeminiRuntimeConfig(configPath)
	if err != nil {
		return err
	}
	...
	for _, item := range []struct {
		...
		{filepath.Join(agentDirectory, geminiGuardScript), "Elydora guard runtime", maxManagedSourceBytes},
		{filepath.Join(agentDirectory, geminiAuditScript), "Elydora audit runtime", maxManagedSourceBytes},
	...
	if !ok || config["agent_name"] != geminiAgentKey ||
		!sameGeminiAgentID(configuredID, agentID) {
		return fmt.Errorf(
			"elydora runtime config identity does not match Gemini CLI agent %s: %s",
			agentID,
			configPath,
		)
	}
...
func geminiRuntimeContractExists(contract geminiRuntimeContract) (bool, error) {
	...
	if !sameGeminiPath(filepath.Dir(agentDirectory), runtimeRoot) ||
		!sameGeminiPath(
			contract.auditPath,
			filepath.Join(agentDirectory, geminiAuditScript),
		) {
		return false, nil
	}
	rootExists, err := managedPhysicalDirectoryExists(
	...
	if err := validateGeminiRuntimeConfig(config, contract.agentID, configPath); err != nil {
		return false, err
	}
...
func geminiRuntimeFilesExist(contracts []geminiRuntimeContract) (bool, error) {
	for _, contract := range contracts {
		exists, err := geminiRuntimeContractExists(contract)
		if err != nil {
			return false, err
		}
		if exists {
