...
export const VALID_PRIVATE_KEY = Buffer.alloc(32, 17).toString('base64url');
export const pluginModuleUrl = pathToFileURL(path.resolve('dist/plugins/kirocli.js')).href;
export const contractModuleUrl = pathToFileURL(
  path.resolve('dist/plugins/kirocli-contract.js'),
).href;
export const installationModuleUrl = pathToFileURL(
  path.resolve('dist/plugins/kirocli-installation.js'),
).href;
export const ioModuleUrl = pathToFileURL(path.resolve('dist/plugins/kirocli-io.js')).href;
export const kiroV1ModuleUrl = pathToFileURL(
...
export async function createFixture({
  ...
  const rootDir = await mkdtemp(path.join(os.tmpdir(), 'elydora-kirocli-'));
  ...
  await writeOptional(v2Path, existingV2);
  await writeOptional(v3Path, existingV3);
  return {
    agentDir,
    agentId,
    baseUrl,
