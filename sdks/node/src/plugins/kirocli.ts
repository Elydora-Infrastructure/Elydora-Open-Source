...
export const kirocliPlugin: AgentPlugin = {
  async install(config: InstallConfig): Promise<void> {
    ...
    preFiltered.push({
      matcher: '*',
      hooks: [{ type: 'command', command: buildHookCommand(config.guardScriptPath), timeout_ms: 5000 }],
    });
    ...
    postFiltered.push({
      matcher: '*',
      hooks: [{ type: 'command', command: buildHookCommand(config.hookScriptPath), timeout_ms: 5000 }],
    });
