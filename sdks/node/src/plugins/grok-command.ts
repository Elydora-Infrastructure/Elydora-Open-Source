...
export interface GrokRuntimeReference {
  readonly agentId: string;
  readonly scriptPath: string;
}

export function sameGrokPath(left: string, right: string): boolean {
...
export function sameGrokAgentId(left: string, right: string): boolean {
  return process.platform === 'win32'
...
export function grokRuntimeReference(
  ...
  if (!sameGrokPath(path.dirname(agentDirectory), path.join(os.homedir(), '.elydora'))) {
    return undefined;
  }
