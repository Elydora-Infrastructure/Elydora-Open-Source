...
export function sameLettaPath(left: string, right: string): boolean {
...
export function sameLettaAgentId(left: string, right: string): boolean {
  return process.platform === 'win32'
...
function runtimeReference(
  ...
  if (!sameLettaPath(path.dirname(agentDirectory), path.join(os.homedir(), '.elydora'))) {
    return undefined;
  }
  const agentId = path.basename(agentDirectory);
  if (!agentId || agentId === '.' || agentId === '..') return undefined;
  return { agentId, executablePath, scriptPath };
}
...
export function lettaRuntimeReference(
  ...
  if (parsed
    ...
    return runtimeReference(parsed[1], scriptName, parsed[0]);
  }
  return undefined;
}
...
export function lettaLegacyRuntimeReference(
  ...
  return legacy ? runtimeReference(legacy, scriptName) : undefined;
}
